...
* **单次有效**：默认 stream 模式下，下载地址在「完整下发整个文件」后立即失效，资源自动释放。
  若下载中途断开（取消下载 / 网络抖动 / 读超时），WebSocket 上传端（浏览器上传页）会保留链接并复位发送位置，可直接重试；TCP 上传端（CLI）的连接无法回卷，中断后需重新运行发送端。
* **断点续传**：默认（stream）模式仍是实时透传，**不支持** Range 与中断续传；如需断点续传请在 provider 使用 `--resumable`，bridge 会将 chunk 落到 `--temp-dir`，下载端原生支持 HTTP `Range`。
  resumable 模式下只有完整下发整个文件才会回收暂存文件；`HEAD` / `Range` / `304` 等探测与分段请求不会消耗链接。
